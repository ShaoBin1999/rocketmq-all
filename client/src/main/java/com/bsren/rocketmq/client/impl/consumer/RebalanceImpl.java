...
import com.bsren.rocketmq.client.consumer.rebalance.AllocateMessageQueueStrategy;
...
import com.bsren.rocketmq.common.protocol.body.LockBatchRequestBody;
import com.bsren.rocketmq.common.protocol.body.UnlockBatchRequestBody;
...
public abstract class RebalanceImpl {
    ...
    //这个是topic对应的messageQueue
    protected final ConcurrentMap<String/* topic */, Set<MessageQueue>> topicSubscribeInfoTable =
            new ConcurrentHashMap<>();

    //这个是topic对应的subscriptionData
    ...
    public void unlock(final MessageQueue mq, final boolean oneway) {
        ...
        if (findBrokerResult != null) {
            UnlockBatchRequestBody requestBody = new UnlockBatchRequestBody(consumerGroup,mQClientFactory.getClientId(),mq);
            try {
    ...
    public void unlockAll(final boolean oneway) {
        ...
        for (final Entry<String, Set<MessageQueue>> entry : brokerMqs.entrySet()) {
            ...
            if (findBrokerResult != null) {
                UnlockBatchRequestBody requestBody = new UnlockBatchRequestBody(consumerGroup,mQClientFactory.getClientId(),mqs);
                try {
                    this.mQClientFactory.getMQClientAPIImpl().unlockBatchMQ(findBrokerResult.getBrokerAddr(), requestBody, 1000, oneway);
    ...
    public boolean lock(final MessageQueue mq) {
        FindBrokerResult findBrokerResult = this.mQClientFactory.findBrokerAddressInSubscribe(mq.getBrokerName(), MixAll.MASTER_ID, true);
        if (findBrokerResult != null) {
            LockBatchRequestBody requestBody = new LockBatchRequestBody(consumerGroup,mQClientFactory.getClientId(),mq);
            try {
    ...
    public void lockAll() {
        ...
        for (Entry<String, Set<MessageQueue>> entry : brokerMqs.entrySet()) {
            ...
            FindBrokerResult findBrokerResult = this.mQClientFactory.findBrokerAddressInSubscribe(brokerName, MixAll.MASTER_ID, true);
            if (findBrokerResult != null) {
                LockBatchRequestBody requestBody = new LockBatchRequestBody(consumerGroup,mQClientFactory.getClientId(),mqs);
    ...
    private void rebalanceByTopic(final String topic, final boolean isOrder) {
        switch (messageModel) {
            ...
            case CLUSTERING: {
                ...
                if (null == mqSet) {
                    if (!topic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX)) {log.warn("doRebalance, {}, but the topic[{}] not exist.", consumerGroup, topic);}
                }
                if (null == cidAll) {
