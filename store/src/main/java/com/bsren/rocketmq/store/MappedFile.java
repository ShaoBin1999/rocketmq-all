 ...
 * 并打上dirty标识，由内核线程flusher定期将被打上dirty的页发送给IO调度层，最后由IO调度决定何时落地到磁盘中，
...
public class MappedFile extends ReferenceResource {
    ...
    public int commit(final int commitLeastPages) {
        ...
        // 如果writeBuffer现在commitPosition已经等于fileSize了，那么把该盘的空间归还掉
    ...
    /**
     * 获取writeBuffer的切片，将commit和write之间的字节写到fileChannel中
     * 更新commitPosition
     */
    ...
    /**
     * 返回一个pos-readPosition之间的一个result
     */
    ...
    public boolean destroy(final long intervalForcibly) {
        ...
        if (this.isCleanupOver()) {  //引用计数已经归零
            try {
                this.fileChannel.close();
     ...
     *  mappedByteBuffer 已经通过 mmap 映射，
     *  此时操作系统中只是记录了该文件和该 Buffer 的映射关系，
     *  而没有映射到物理内存中。
